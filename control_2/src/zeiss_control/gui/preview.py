...
class Preview(QWidgetRestore):
    ...
    def __init__(self, parent: QWidget | None = None, mmcore: CMMCorePlus | None = None,
        ...
        self.connect_events()
        self._mmc.mda.events.sequenceStarted.connect(self.disconnect_events)
        self._mmc.mda.events.sequenceFinished.connect(self.connect_events)
...
class Canvas(QWidget):
    ...
    def __init__(
        ...
        *,
        ...
        self.percentile_clim = QLineEdit(str(self.vis_settings[self.last_channel]["clim_percentile"]))
        self.percentile_clim.setMaximumWidth(30)
        self.percentile_clim.editingFinished.connect(self.update_percentile)
        ...
        self.clim_layout.addWidget(self.percentile_clim)
        ...
        # timer for percentile clim update
        self.clim_timer = QtCore.QTimer(parent=self)
        self.clim_timer.setInterval(500)
        self.clim_timer.timeout.connect(self._calculate_clims)
    ...
    def update_auto(self, state: int) -> None:
        if state == 2:
            self.vis_settings[self.last_channel]["clim_mode"]= "auto"
        elif state == 0:
            self.vis_settings[self.last_channel]["clim_mode"] = "manual"

    def update_percentile(self):
        self.auto_clim.setChecked(True)
        val_percentile = self.percentile_clim.text()
        try: 
            self.vis_settings[self.last_channel]["clim_percentile"] = float(val_percentile)
        except ValueError:
            self.vis_settings[self.last_channel]["clim_percentile"] = 0
            self.percentile_clim.setText("0")

    def _adjust_channel(self, channel: str) -> None:
        if channel == self.last_channel:
            return
        if channel not in self.vis_settings.keys():
            self.vis_settings[channel] = self.vis_settings[0]
        # self.histogram.set_max(self._clims.get(channel, (0, 2))[1])
        block = self.clim_slider.blockSignals(True)
        self.clim_slider.setMaximum(self.vis_settings[channel]["clims"][1])
        self.clim_slider.blockSignals(block)
        block = self.auto_clim.blockSignals(True)
        self.auto_clim.setChecked(self.vis_settings[channel]["clim_mode"] == "auto")
        self.auto_clim.blockSignals(block)
        self.percentile_clim.blockSignals(True)
        self.percentile_clim.setText(self.vis_settings[channel]["clim_percentile"])
        self.percentile_clim.blockSignals(False)

    def _calculate_clims(self):
        channel = self._mmc.getCurrentConfig("Channel")
        self._adjust_channel(channel)
        clim_max = np.percentile(self.img, self.vis_settings[channel]["clim_percentile"])
        clim_min = np.percentile(self.img, 100 - self.vis_settings[channel]["clim_percentile"])
        clim = (clim_min, clim_max)
        self.vis_settings[channel]["clims"] = clim
    ...
    def _on_image_snapped(self, img: np.ndarray | None = None, channel: str|None = None) -> None:
        ...
        if self.vis_settings[channel]["clim_mode"] == "auto":
            clim = self.vis_settings[channel]["clims"]
        else:
            clim = self.clim_slider.value()
        if self.image is None:
            self.image = self._imcls(
                img, cmap=self.vis_settings[channel]["cmap"], clim=clim, parent=self.view.scene
            )
